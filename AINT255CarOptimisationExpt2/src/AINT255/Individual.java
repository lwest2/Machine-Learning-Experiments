...
public class Individual {
    ...
    public void crossOver(Individual parent2, double probabilityCrossover) {
          ...
          {
              // scaling factor berween 0 and 1
              scalingFactor = (random() * (upper - lower)) + lower;
              
              // offspring = p1 and p2 child
              this.simulationParameters[i] = scalingFactor * this.simulationParameters[i] + (1 - scalingFactor) * parent2.simulationParameters[i];
              
              // scaling factor again
              scalingFactor = (random() * (upper - lower)) + lower;
              // offspring = p2 and p1 child
              parent2.simulationParameters[i] = scalingFactor * parent2.simulationParameters[i] + (1 - scalingFactor) * this.simulationParameters[i];       
          }        
          }
    }
